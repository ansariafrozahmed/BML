...
const EditAccountDetails = ({ userData }: any) => {
    ...
    const ensurePhoneAndWhatsapp = () => {
        ...
        if (!updatedContactDetails.some((contact: any) => contact.label === 'Phone')) {
            updatedContactDetails.push({ label: 'Phone', value: '' });
        }
        if (!updatedContactDetails.some((contact: any) => contact.label === 'Whatsapp Number')) {
    ...
    const handleContactFieldChange = (value: string, label: string) => {
        ...
        const updatedContactDetails = (formData?.contact_details || []).map((contact: any) => {
            ...
            <Form layout="vertical" scrollToFirstError>
                ...
                <Form.Item label="First Name">
                    <Input
                        ...
                        onChange={(e) => handleDebouncedChange('first_name', e.target.value)}
                    />
                </Form.Item>
                ...
                {contactFields.map((contact: any) => (
                    <Form.Item label={contact.label} key={contact.label}>
                        <Input
                            ...
                            onChange={(e) => handleContactFieldChange(e.target.value, contact.label)}
                        />
                    </Form.Item>
                ))}
